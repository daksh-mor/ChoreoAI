...
#### Solution
I analyzed reference code examples and identified the semantic meaning of each joint and observed that the z axis was inverted in the given data. I created a mapping of 53 relevant points (removing 2 redundant points at indices `[26, 53]` that were nearly overlapping with others):
...
#### Solution
To ensure the dancer remains in view, I dynamically adjusted the camera position based on the movement of the skeleton. Instead of keeping the camera static, I calculated the center of the current frame and updated the camera position accordingly. The update starts after detecting movement beyond a small threshold or after 200 frames to prevent premature adjustments. This approach ensures that even in highly dynamic sequences, the dancer stays within the frame, improving visualization clarity.
